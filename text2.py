...
if query_name:
    ...
    # 整合两个数据表，用于绘制折线图
    if not result_new2_2.empty and not result_new2_1.empty:
        combined_df = pd.concat([result_new2_2, result_new2_1], ignore_index=True)
        fig = px.line(combined_df, x='作者', y='失信指数', title='科研人员失信指数')
        st.plotly_chart(fig)
    else:
